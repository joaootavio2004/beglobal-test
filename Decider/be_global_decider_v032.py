...
# -*- coding: utf-8 -*-
"""
Be Global — Decision Helper (v0.3.0)
------------------------------------
Novidades:
• Base de mercado configurável: Population (padrão) ou Demand (aba “Market Analyzes”).
• Parâmetros específicos para países já presentes: k_owned e pmax_owned_mult.
• Objetivo “blended” (lucro do ano + valor da base do próximo ano): alpha*Lucro + (1-alpha)*rho*Base*P*ARPU.
• Mantém: parser robusto, merge por país normalizado, auto-scale de unidades, JV/FDI, gate 2.4× por quantil de GDPpc.
"""
...
# ------------------------
# GDP/Population parsing (v0.2.8 core)
# ------------------------
import re as _re
...
def extract_demand_table(xls, sheet="Market Analyzes") -> pd.DataFrame:
    ...
    if ccol is None:
        ccol = df.columns[0]
    # demand-like column: "Demand", "Demanda", "Market Demand", "Demand (M)" etc.
    dcol = None
    for c in df.columns:
        cl = c.lower()
        if any(k in cl for k in ["demand","demanda","market demand","market size","demanda de mercado","tamanho do mercado"]):
            dcol = c; break
    # se não achar, tenta alguma coluna grande e numérica
    if dcol is None:
        num_cols = []
...
def parse_price_overrides(s: Optional[str], price_cats: List[Tuple[str,float]]) -> Dict[str, float]:
    ...
    if not s: return {}
    name_to_mult = {}
    # Map normalized category names (with/without accents) to multiplier
    cat_map = {}
    for nm, mult in price_cats:
        cat_map[_normalize_txt(nm)] = mult
    # Portuguese synonyms
    cat_syn = {
        "medio":"médio", "med":"médio",
        "muito alto":"muito alto", "alto":"alto", "baixo":"baixo", "muito baixo":"muito baixo",
    ...
    for token in s.split(","):
        ...
        if mult is None:
            # fallback: try closest by prefix
...
# ------------------------
# Core model (v0.3.0 with blended objective & base_size)
# ------------------------
...
# ------------------------
# Cash & index (idem v0.2.8, com tweaks)
# ------------------------
...
def read_carryover_from_general_report(xls, group: int, year: int) -> Optional[float]:
    """Tenta encontrar uma taxa de 'carry over' para usar como rho (retenção)."""
    try:
        gr = pd.read_excel(xls, sheet_name="General Report", header=None)
    except Exception:
        return None
...
def decide_for_group(xlsx_path: str, group: int, decision_year: int, cash_cap_pct: float=0.50,
                     ...
                     min_mkt_owned_pc: float=0.0,
                     min_mkt_owned_total: float=0.0,
                     lock_price_owned: bool=False,
                     target_share: str=None,
    ...
    if not np.isfinite(gdppc_median) or gdppc_median <= 0:
        ...
        if not np.isfinite(gdppc_median) or gdppc_median <= 0:
            gdppc_median = 1.0
            if debug:
                print("[hint] GDPpc mediana <= 0: fallback=1.0")
    ...
    override_map = parse_price_overrides(price_override, price_cats) if price_override else {}

    # Target share map (normalized country -> target fraction 0..1)
    ts_map = {}
    if target_share:
    ...
    if ma_quantile >= 0.0:
        ...
        for c in macro_y1.index:
            try:
                ...
                pass
    # rho (carryover) tentativa
    rho = read_carryover_from_general_report(xls, group, decision_year) or rho_default



    def penalized_value(est: Dict[str,float], base_size: float, country_name: str, objective: str, alpha: float, rho: float) -> float:
        if objective == "profit":
            val = est["NetIncome"]
        else:
            growth_value = rho * base_size * est["P"] * est["ARPU"]
            val = alpha*est["NetIncome"] + (1.0-alpha)*growth_value
        # apply share shortfall penalty if configured and country has a target
        if share_penalty and share_penalty>0 and country_name is not None:
            targ = ts_map.get(normalize_country(country_name))
            if targ is not None and targ > 0:
                shortfall = max(0.0, targ - est["P"])
                # penalize in revenue-equivalent units
    ...
    for c in owned:
        ...
        # Build candidate price list, honoring overrides
        cat_list = price_cats
        if normalize_country(c) in override_map:
            forced_mult = override_map[normalize_country(c)]
            # find matching category name for printing
            forced_name = next((nm for nm,m in price_cats if abs(m-forced_mult)<1e-9), f"forced({forced_mult:.2f}x)")
            if lock_price_owned:
                cat_list = [(forced_name, forced_mult)]
            else:
                # put forced one first
                other = [(nm,m) for (nm,m) in price_cats if abs(m-forced_mult)>=1e-9]
                cat_list = [(forced_name, forced_mult)] + other
        ...
        for cat, price_mult in cat_list:
            ...
            # optimize mkt_pc via golden section manually to apply custom penalty
            def _obj(x):
                ...
                return penalized_value(est_tmp, base_size, c, objective, alpha_blend, rho)
            # golden section
            gr = (math.sqrt(5) - 1)/2
            a,b = 0.0, p_max
            c1 = b - gr*(b-a); d1 = a + gr*(b-a)
            f_c1 = _obj(c1); f_d1 = _obj(d1)
            while abs(b-a) > 1e-5:
                if f_c1 < f_d1:
                    a = c1; c1 = d1; f_c1 = f_d1; d1 = a + gr*(b-a); f_d1 = _obj(d1)
                else:
                    b = d1; d1 = c1; f_d1 = f_c1; c1 = b - gr*(b-a); f_c1 = _obj(c1)
            mkt_pc_opt = (a+b)/2.0
            est = estimate_profit(
                ...
                epsilon=epsilon, p_max=use_pmax, k=use_k, gamma_comp=gamma_comp, score_mode=score_mode,
                fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005), opening_rate=0.0, tax_rate=rr.get('tax_rate',0.20),
                apply_opening=False, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc,
                objective=objective, alpha=alpha_blend, rho=rho, lo=0.0, hi=p_max, tol=1e-5
            )
            # Enforce minimum MKT_pc if requested
            if min_mkt_owned_pc and mkt_pc_opt < min_mkt_owned_pc:
                est = estimate_profit(
                    base_size=base_size, score=score, competitors=competitors, mkt_pc=min_mkt_owned_pc,
                    price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon,
                    p_max=use_pmax, k=use_k, gamma_comp=gamma_comp, score_mode=score_mode,
                    fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005), opening_rate=0.0, tax_rate=rr.get('tax_rate',0.20),
                    apply_opening=False, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc
        ...
        owned_rows.append({
            ...
            "MKT_total": (best["MKT_pc"]*base_size) if best else 0.0,
    ...
    # Enforce total minimum marketing spend across owned, if requested
    if not df_owned.empty and min_mkt_owned_total and min_mkt_owned_total > 0:
        ...
        if current < min_mkt_owned_total:
            # scale all owned proportionally up to the minimum
            scale = (min_mkt_owned_total / max(current, 1e-9))
            df_owned["MKT_total"] *= scale
            # recompute MKT_pc
            df_owned["MKT_pc"] = df_owned.apply(lambda r: r["MKT_total"] / max(1.0, base_for_country(r["Country"])), axis=1)
    ...
    if cash_cap is not None and not df_owned.empty:
        ...
        if total_owned > cash_cap and total_owned > 0:
            ...
            df_owned["MKT_total"] *= scale
            # recomputa MKT_pc com base_size
            df_owned["MKT_pc"] = df_owned.apply(lambda r: r["MKT_total"] / max(1.0, base_for_country(r["Country"])), axis=1)
    ...
    for c in candidates:
        ...
        new_rows.append({
            ...
            "MKT_pc": best["MKT_pc"],
            "MKT_total": best["MKT_pc"]*base_size,
            "Est_P": best["P"],
            "Est_ARPU": best["ARPU"],
...
def main():
    ...
    ap.add_argument("--target_share", type=str, default=None,
                    help="Ex: 'United States:0.05,Canada:0.02' (fração 0-1 por país)")
    ap.add_argument("--share_penalty", type=float, default=0.0,
                    help="Peso da penalidade (em unidades ~Receita): penaliza shortfall de share")
    ...
    metrics, df_owned, df_new, cash_cap, owned, remaining_cash, comp_price, gdppc_median, eps, rho, base_source = decide_for_group(
        ...
        alpha_blend=args.alpha_blend, rho_default=args.rho_default, debug=args.debug,
        price_override=args.price_override, min_mkt_owned_pc=args.min_mkt_owned_pc,
        min_mkt_owned_total=args.min_mkt_owned_total, lock_price_owned=args.lock_price_owned,
        target_share=args.target_share, share_penalty=args.share_penalty
    )
