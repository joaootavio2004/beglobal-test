...
def build_macro(xls, pop_sheet: str, country_col: str, year_header_row: int,
    ...
    df = pd.merge(pop.drop(columns=["Country"]), gdp.drop(columns=["Country"]), on=["NormCountry","Year"], how="left")
...
def main():
    ...
    print("\nPaíses já presentes do grupo:", ", ".join(owned) if owned else "(nenhum)")
