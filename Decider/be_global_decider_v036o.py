...
def decide_for_group(xlsx_path: str, group: int, decision_year: int, cash_cap_pct: float=0.50,
    ...
    # --- Backfill por país (se GDP/Pop ausente em Y+1, pega ano mais próximo) ---
    if not macro_y1.empty:
        fixed_rows = []
        for cname in list(macro_y1.index):
            row = macro_y1.loc[cname]
            need_pop = (pd.isna(row.get("Population")) or float(pd.to_numeric(row.get("Population"), errors="coerce") or 0.0) <= 0.0)
            need_gdp = (pd.isna(row.get("GDP")) or float(pd.to_numeric(row.get("GDP"), errors="coerce") or 0.0) <= 0.0)
            if need_pop or need_gdp:
                g = macro_long[macro_long["Country"].str.lower()==str(cname).lower()].copy()
                if not g.empty:
                    g["gap"] = (g["Year"] - (decision_year+1)).abs()
                    r = g.sort_values("gap").iloc[0]
                    pop = float(pd.to_numeric(r.get("Population"), errors="coerce") or 0.0)
                    gdp = float(pd.to_numeric(r.get("GDP"), errors="coerce") or 0.0)
                    if pop>0: macro_y1.at[cname,"Population"] = pop
                    if gdp>0: macro_y1.at[cname,"GDP"] = gdp
        # recomputa GDPpc após possíveis preenchimentos
        macro_y1["GDP"] = pd.to_numeric(macro_y1["GDP"], errors="coerce")
        macro_y1["Population"] = pd.to_numeric(macro_y1["Population"], errors="coerce")
        macro_y1["GDPpc"] = macro_y1["GDP"] / macro_y1["Population"]
    ...
    owned_rows = []
    for c in owned:
        # contexto garantido para país OWNED
        ...
        region = country_region.get(c_norm, None)
        ...
        if c in override_map_canon:
            forced_mult = override_map_canon[c]; applied_override = True
        elif c_norm in override_map_norm:
            forced_mult = override_map_norm[c_norm]; applied_override = True

        if forced_mult is not None:
            # mantém o multiplicador forçado na frente da lista
            forced_name = None
            for nm, mult in price_cats:
                if abs(mult - forced_mult) < 1e-9:
                    forced_name = nm; break
            if forced_name is None:
                forced_name = "Override"
            cat_list = [(forced_name, forced_mult)] + [(nm, m) for (nm, m) in price_cats if abs(m - forced_mult) >= 1e-9]
        else:
            cat_list = price_cats

        if lock_price_owned and forced_mult is not None:
        ...
        for cat, price_mult in cat_list:
            ...
            mkt_pc_opt, est = argmax_mkt_pc(
                base_size=base_size, score=score, competitors=competitors,
                price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon,
                p_max=use_pmax, k=use_k, gamma_comp=gamma_comp, score_mode=score_mode,
                fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005),
                opening_rate=0.0, tax_rate=rr.get('tax_rate',0.20),
                apply_opening=False, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc,
                lo=0.0, hi=use_pmax, tol=1e-5
            )
            ...
            if min_mkt_owned_pc and mkt_pc_opt < min_mkt_owned_pc:
                ...
                est = estimate_profit(
                    ...
                    fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005),
                    opening_rate=0.0, tax_rate=rr.get('tax_rate',0.20),
                    apply_opening=False, comp_price=comp_price, gdp_aff=gdp_aff,
        ...
        # GDPpc fallback robusto
        if "GDPpc" in macro_y1.columns:
            gdpv = pd.to_numeric(macro_y1.at[c,"GDPpc"], errors="coerce")
        else:
            gdpv = None
        if gdpv is None or pd.isna(gdpv) or float(gdpv)==0.0:
            g = pd.to_numeric(macro_y1.at[c,"GDP (million US$)"], errors="coerce") if "GDP (million US$)" in macro_y1.columns else None
            p = pd.to_numeric(macro_y1.at[c,"Population"], errors="coerce") if "Population" in macro_y1.columns else None
            if (g is not None) and (p is not None) and pd.notna(g) and pd.notna(p) and float(p)>0:
                gdpv = (float(g)*1_000_000.0)/float(p)/1000.0
            else:
                gdpv = float(gdppc_median)

        owned_rows.append({
            "Country": c,
            "Competitors(Y+1)": int(competitors),
            "Score(0-100)": round(float(score),1),
            "GDPpc": float(gdpv),
            ...
            "Est_P": best["P"] if best is not None else None,
            "Est_ARPU": best["ARPU"] if best is not None else None,
            "Est_Revenue": best["Revenue"] if best is not None else None,
            "Est_NetIncome": best["NetIncome"] if best is not None else None,
            "AppliedOverride": applied_override
        })

    df_owned = pd.DataFrame(owned_rows)
    if not df_owned.empty:
        df_owned["Country"] = df_owned["Country"].astype(str)
    ...
    for c in candidates:
        ...
        # limite por caixa remanescente
        if remaining_cash is None:
            hi_pc_cap = p_max
        else:
            hi_pc_cap = min(p_max, max(0.0, remaining_cash / max(base_size,1.0)))
        ...
        for cat, price_mult in price_cats:
            ...
            # FDI
            mkt_fdi, est_fdi = argmax_mkt_pc(
                base_size=base_size, score=score, competitors=competitors,
                price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon,
                p_max=hi_pc_cap, k=k, gamma_comp=gamma_comp, score_mode=score_mode,
                fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005),
                opening_rate=rr.get('opening_rate',0.03), tax_rate=rr.get('tax_rate',0.20),
                apply_opening=True, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc,
                lo=0.0, hi=hi_pc_cap, tol=1e-5
            )

            # JV (abertura pela metade, lucro pela metade)
            mkt_jv, est_jv = argmax_mkt_pc(
                base_size=base_size, score=score, competitors=competitors,
                price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon,
                p_max=hi_pc_cap, k=k, gamma_comp=gamma_comp, score_mode=score_mode,
                fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005),
                opening_rate=0.5*rr.get('opening_rate',0.03), tax_rate=rr.get('tax_rate',0.20),
                apply_opening=True, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc,
                lo=0.0, hi=hi_pc_cap, tol=1e-5
            )
            est_jv["NetIncome"] *= 0.5

            # objetivo
            def metric(d): 
                return d["NetIncome"] if objective=="profit" else (d["Revenue"] if objective=="revenue" else (alpha_blend*d["NetIncome"] + (1-alpha_blend)*d["Revenue"]))

            cand_mode, cand_mkt, cand_est = ("FDI", mkt_fdi, est_fdi) if metric(est_fdi)>=metric(est_jv) else ("JV", mkt_jv, est_jv)

            # respeita caixa
            mkt_total = cand_mkt * base_size
            if remaining_cash is not None and mkt_total > remaining_cash + 1e-6:
                cand_mkt = max(0.0, remaining_cash / max(base_size,1.0))
                cand_est = estimate_profit(
                    base_size=base_size, score=score, competitors=competitors, mkt_pc=cand_mkt,
                    price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon,
                    p_max=hi_pc_cap, k=k, gamma_comp=gamma_comp, score_mode=score_mode,
                    fixed_rate=rr.get('fixed_rate',0.02), reg_rate=rr.get('reg_rate',0.005),
                    opening_rate=rr.get('opening_rate',0.03) if cand_mode=="FDI" else 0.5*rr.get('opening_rate',0.03),
                    tax_rate=rr.get('tax_rate',0.20), apply_opening=True,
                    comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc
                )
                if cand_mode=="JV":
                    cand_est["NetIncome"] *= 0.5

            if (best is None) or (metric(cand_est) > metric(best)):
                best = cand_est.copy(); best_cat=(cat, price_mult); best_margin=margin_pct; best_mode=cand_mode; best_pen=cand_est["PricePenalty"]; best_mkt_pc=cand_mkt
