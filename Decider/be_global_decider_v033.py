...
# -*- coding: utf-8 -*-
"""
Be Global — Decision Helper (v0.3.3)
------------------------------------
• Base de mercado configurável: Population ou Demand (Market Analyzes).
• Parâmetros específicos para países já presentes: k_owned e pmax_owned_mult.
• Objetivo "blended" (lucro do ano + valor da base do próximo ano).
• Override/lock de preço por país nos ativos, MKT mínimo por país e total.
• Meta de market share por país (soft-constraint) com penalidade.
"""
...
def extract_demand_table(xls, sheet="Market Analyzes") -> pd.DataFrame:
    ...
    for c in df.columns:
        ...
        if any(k in cl for k in ["demand","demanda","market demand","market size","demanda de mercado","tamanho do mercado"]):
            dcol = c; break
    if dcol is None:
        # fallback: maior coluna numérica
        num_cols = []
...
def decide_for_group(xlsx_path: str, group: int, decision_year: int, cash_cap_pct: float=0.50,
    ...
    owned = sorted(dict.fromkeys(owned))  # dedup
    ...
    for c in owned:
        ...
        for cat, price_mult in cat_list:
            ...
            # golden-section optimize with custom penalty
            gr = (math.sqrt(5) - 1)/2
            a, b = 0.0, p_max
            ...
            # enforce per-capita min if requested
    ...
    # enforce total minimum across owned
