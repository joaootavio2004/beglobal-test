...
def decide_for_group(xlsx_path: str, group: int, decision_year: int, cash_cap_pct: float=0.50,
    ...
    for c in owned:
        ...
        # === OVERRIDE de preço/margem para países OWNED ===
        applied_override = False
        forced_label = None
        c_norm = normalize_country(c)

        # Resolve rótulo a partir de --price_override quando --lock_price_owned está ativo
        if lock_price_owned:
            if c in override_map_canon:
                forced_label = override_map_canon[c]
            elif c_norm in override_map_norm:
                forced_label = override_map_norm[c_norm]

        PRICE_LABEL_TO_MULT = {
            "Muito Baixo": 0.6, "Baixo": 0.8, "Médio": 1.0, "Alto": 1.2, "Muito Alto": 2.4,
            "muito baixo": 0.6, "baixo": 0.8, "medio": 1.0, "médio": 1.0, "alto": 1.2, "muito alto": 2.4,
            "very low": 0.6, "low": 0.8, "medium": 1.0, "high": 1.2, "very high": 2.4,
        }
        MARGIN_BY_LABEL = {
            "Muito Baixo":0.10, "Baixo":0.20, "Médio":0.25, "Alto":0.30, "Muito Alto":0.40,
            "muito baixo":0.10, "baixo":0.20, "medio":0.25, "médio":0.25, "alto":0.30, "muito alto":0.40,
            "very low":0.10, "low":0.20, "medium":0.25, "high":0.30, "very high":0.40,
        }

        if forced_label is not None:
            price_mult = PRICE_LABEL_TO_MULT.get(forced_label, 1.0)
            margin_pct = MARGIN_BY_LABEL.get(forced_label, 0.25)
        ...
        for cat, price_mult in cat_list:
            ...
            def obj(x, base_size=base_size, score=score, competitors=competitors, price_mult=price_mult, margin_pct=margin_pct, epsilon=epsilon, use_pmax=use_pmax, use_k=use_k, gamma_comp=gamma_comp, score_mode=score_mode, rr=rr, comp_price=comp_price, gdp_aff=gdp_aff, eta_gap=eta_gap, eta_disc=eta_disc):
