...
def decide_for_group(xlsx_path: str, group: int, decision_year: int, cash_cap_pct: float=0.50,
    ...
    for c in owned:
        ...
        applied_override = False  # will be set True if forced_mult detected
        forced_mult = None
        forced_name = None
        if c in override_map_canon:
            ...
            forced_mult = override_map_norm[c_norm]
            applied_override = True
        ...
        for cat, price_mult in cat_list:
            ...
            # (substituído) usa argmax_mkt_pc direto, sem fechamento de variáveis
...
if pd.isna(gdpv) or float(gdpv) == 0.0:
...
owned_rows.append({
    ...
    if not df_owned.empty:
        df_owned = df_owned.sort_values(["Country","Est_NetIncome"], ascending=[True,False]).drop_duplicates(subset=["Country"], keep="first").drop_duplicates(subset=["Country"], keep="first")
