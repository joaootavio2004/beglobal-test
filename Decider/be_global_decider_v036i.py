    ...
    for c in owned:
        ...
        owned_rows.append({
            ...
            if pd.isna(gdpv) or float(gdpv) == 0.0:
                g = pd.to_numeric(macro_y1.at[c, "GDP (million US$)"] if "GDP (million US$)" in macro_y1.columns else None, errors="coerce")
                p = pd.to_numeric(macro_y1.at[c, "Population"] if "Population" in macro_y1.columns else None, errors="coerce")
